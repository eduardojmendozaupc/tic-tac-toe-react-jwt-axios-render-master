...
export default function Game() {
  ...
  useEffect(() => {
    ...
    if (!t) {
      ...
      const getWinners = async () => {
        const response = await axios.get("https://tic-tac-toe-master.onrender.com/api/v1/winners", {
          headers: {
  ...
  const handleWinner = async (w) => {
    ...
    try {
      const t = localStorage.getItem("jwt-token");
      const current_winner = await axios.post("https://tic-tac-toe-master.onrender.com/api/v1/winners", winner, {
        headers: {
          Authorization: `Bearer ${t}`,
        },
      });
